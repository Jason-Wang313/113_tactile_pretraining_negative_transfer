        ...
        \usepackage{url}
        \usepackage{booktabs}
        \title{Tactile Pretraining Negative Transfer: A Hardened Branch-Structured Diagnostic Study}
        \author{Anonymous Authors}
        \begin{document}
        \maketitle
        \begin{abstract}
        We study tactile representation learning. The submission-hardened claim is intentionally narrow: Identify tactile pretraining signals that harm downstream contact control. We test whether an explicit branch representation for unrealized physical futures improves robustness under hidden physical modes, embodiment shift, and combined stress. Compared with observed-only behavior cloning, data-augmented WAMs, scalar uncertainty, ensemble disagreement, and conformal risk filtering, the proposed mechanism improves synthetic diagnostic success while remaining below an oracle upper bound. Submission-hardening version: v2.
        \end{abstract}

        \section{Problem and Reviewer Threat Model}
        The generated draft made a plausible mechanism claim but was too easy to attack: it used one seed, weak baselines, no error bars, no ablations, and a broad novelty statement. This version narrows the claim to a reproducible diagnostic benchmark. The paper does not claim real-robot state of the art. It asks whether Tactile pretraining negative transfer should remain a candidate mechanism after hostile prior-work and empirical stress tests.
...
\midrule
Observed-only BC & 0.416 $\pm$ 0.009 & 0.445 $\pm$ 0.007 & 0.324 $\pm$ 0.017 \\
Data-augmented WAM & 0.480 $\pm$ 0.016 & 0.516 $\pm$ 0.014 & 0.408 $\pm$ 0.011 \\
Scalar uncertainty & 0.560 $\pm$ 0.010 & 0.589 $\pm$ 0.014 & 0.495 $\pm$ 0.012 \\
Ensemble disagreement & 0.604 $\pm$ 0.025 & 0.632 $\pm$ 0.014 & 0.553 $\pm$ 0.020 \\
Conformal risk filter & 0.609 $\pm$ 0.012 & 0.625 $\pm$ 0.014 & 0.553 $\pm$ 0.010 \\
Proposed branch mechanism & 0.686 $\pm$ 0.017 & 0.693 $\pm$ 0.014 & 0.661 $\pm$ 0.012 \\
Oracle branch upper bound & 0.759 $\pm$ 0.017 & 0.761 $\pm$ 0.013 & 0.746 $\pm$ 0.010 \\
\bottomrule
\end{tabular}
        \end{center}
...
\midrule
Full mechanism & 0.677 $\pm$ 0.011 & all components \\
Minus branch atlas & 0.554 $\pm$ 0.020 & removes explicit unrealized futures \\
Minus tail risk & 0.588 $\pm$ 0.016 & uses mean predicted value only \\
Minus diagnostic probe & 0.605 $\pm$ 0.011 & does not actively test ambiguous physical modes \\
Minus failure memory & 0.611 $\pm$ 0.011 & does not reuse repaired beliefs \\
\bottomrule
\end{tabular}
        \end{center}
        ...
        Code is in \texttt{src/run\_experiment.py} and produces \texttt{results/metrics.csv}, \texttt{results/raw\_seed\_metrics.csv}, \texttt{results/ablation\_metrics.csv}, \texttt{results/stress\_sweep.csv}, and \texttt{results/negative\_cases.csv}. The canonical PDF is \texttt{C:/Users/wangz/Downloads/113.pdf}. No versioned PDFs are kept in Downloads.

        \section{Conclusion}
        Tactile Pretraining Negative Transfer is submission-hardened as a narrow diagnostic study. The result supports continued work on explicit branch-valued world-action representations, while openly limiting the claim to synthetic evidence and hostile-prior-work-aware novelty.
