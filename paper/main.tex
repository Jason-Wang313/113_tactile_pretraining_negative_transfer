...
The failure-case audit records eight boundaries: unseen taxel dead zones, ambiguous texture aliasing, catastrophic source mislabels, shear without normal force, beneficial pretraining rejected too aggressively, contact geometry outside the source family, latency masking transient slip, and remaining oracle headroom under compound shift. These cases are the reason the paper must remain a strong-revise project: a real submission needs sensor-health modeling, hardware timing checks, geometry-conditioned tactile transfer validation, and robot or independent high-fidelity rollouts.
